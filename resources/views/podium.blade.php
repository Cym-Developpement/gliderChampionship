...
<head>
    ...
    <title>Podium journée — {{ config('app.name') }}</title>
    <style>
...
<!-- Classement complet (4ème et au-delà) -->
...
async function loadPodium() {
    ...
    // Classement général (toutes journées)
    const res = await fetch(GENERAL_RANKING_URL, { cache: 'no-store' });
    if (!res.ok) return;
    const data = await res.json();

    // Trouver le dernier jour disponible
    ...
    if (lastDay !== null) {
        ...
        dayLabel = `Scores du jour ${lastDay}`;
        if (!isDayValidated) dayLabel += ' · PROVISOIRE';
    }
    document.getElementById('compSub').textContent = dayLabel;
    if (!isDayValidated && lastDay !== null) {
        document.getElementById('compSub').style.color = '#ff6b6b';
    }

    // Trier par score du dernier jour
    const pilots = (data.pilots || []).map(p => ({
        ...p,
        dayScore: (p.dayScores || {})[lastDay] ?? 0,
        dayValidated: (p.dayValidated || {})[lastDay] === true,
    })).sort((a, b) => b.dayScore - a.dayScore);

    const top3 = pilots.slice(0, 3);

    // Ordre d'affichage: 2 - 1 - 3
    const order = [
    ...
    for (const { rank, pilot } of order) {
        ...
        if (pilot) {
            const provTag = !isDayValidated
                ? `<div style="font-size:0.55rem;font-weight:700;letter-spacing:1.5px;color:#ff6b6b;text-transform:uppercase;margin-bottom:0.2rem;">Provisoire</div>`
                : '';
            step.innerHTML = `
                <div class="rank-number">#${rank}</div>
                <div class="pilot-name">${pilot.name}</div>
                ${pilot.callsign ? `<div class="pilot-callsign">${pilot.callsign}</div>` : ''}
                <div class="pilot-score">${pilot.dayScore.toLocaleString('fr-FR')}</div>
                <div class="pilot-score-label">pts jour ${lastDay}</div>
                ${provTag}
            `;
            if (pilot.photoUrl) {
    ...
    // Classement complet (4ème et au-delà) — trié par score du jour, colonne unique jour
    ...
    if (rest.length > 0) {
        ...
        cols.forEach((col, ci) => {
            ...
            const provHeader = !isDayValidated
                ? ` <span style="font-size:0.6rem;color:#ff6b6b;font-weight:700;">PROVISOIRE</span>`
                : '';
            let html = `<table class="ranking-table">
                <thead><tr>
                    <th colspan="2">Pilote</th>
                    <th>Jour ${lastDay}${provHeader}</th>
                </tr></thead>
                <tbody>`;
            ...
            col.forEach((p, idx) => {
                ...
                html += `<tr>
                    <td style="width:32px"><span class="rank-badge">${rank}</span></td>
                    <td>
                    ...
                    <td class="total-cell">${(p.dayScore ?? 0).toLocaleString('fr-FR')}</td>
                </tr>`;
            });
